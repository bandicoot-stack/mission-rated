...
for (const file of ['index.html','military-value.html']) {
  ...
  if (file === 'index.html' && !html.includes('labor-day-deal-instrumentation.js')) html=html.replace('</body>','<script src="/labor-day-deal-instrumentation.js" defer></script>\n</body>');
...
{
  const path='dist/savings.html';let html=await readFile(path,'utf8');for(const asset of ['deal-share.js','savings-share.js'])if(!html.includes(asset))html=html.replace('</body>',`<script src="/${asset}" defer></script>\n</body>`);await writeFile(path,html);
}
