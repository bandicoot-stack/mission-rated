...
function enforce(){
  ...
  const partner=window.MRFeaturedPartners?.get('yorktown-tools');
  if(!partner){console.error('Featured partner data missing: yorktown-tools');return;}
  ...
  featured.innerHTML=`
    ...
    <div class="grid"><article class="card hot" data-featured-partner="${escapeHtml(partner.slug)}" style="grid-column:1/-1;border-color:#8d742a;background:linear-gradient(135deg,#102234,#081723)">
      ...
      <h3 style="font-size:24px">${escapeHtml(partner.name)}</h3><div class="offer" style="font-size:42px">${escapeHtml(partner.offer)}</div><div class="line"><b>${escapeHtml(partner.offerLabel)}</b></div>
      <p class="muted">${escapeHtml(partner.description)}</p>
      <div class="links"><a class="btn" style="background:#00e5ff;color:#02101d;border-color:#00e5ff" href="${escapeHtml(partner.featuredUrl)}">View featured partner →</a><a class="btn" href="${escapeHtml(partner.businessUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(partner.name)} ↗</a></div>
    </article></div>`;
  window.MRPartnerLogo?.render(featured.querySelector('[data-partner-logo-host]'),partner,{eager:true});
