...
function interstitials(){
  const nav=tabsNav(),main=mainEl();
  if(!nav||!main) return [];
  const out=[];let n=nav.nextElementSibling;
  while(n&&n!==main){out.push(n);n=n.nextElementSibling;}
  return out;
}
function setInterstitialVisibility(show){
  for(const el of interstitials()){
    if(show){
      if(el.dataset.mrFeaturedPrevDisplay!==undefined){el.style.display=el.dataset.mrFeaturedPrevDisplay;delete el.dataset.mrFeaturedPrevDisplay;}
    }else{
      if(el.dataset.mrFeaturedPrevDisplay===undefined) el.dataset.mrFeaturedPrevDisplay=el.style.display||'';
      el.style.display='none';
    }
  }
}
function purgeLeak(){
  if(!featuredActive()) return;
  setInterstitialVisibility(false);
  document.querySelectorAll('main.main > .section').forEach(s=>s.hidden=s.id!=='featured');
}
function enforce(){
  const main=mainEl(),tabs=document.querySelector('.tabrow');
  if(!main||!tabs) return;
  let featured=document.getElementById('featured');
  if(!featured){featured=document.createElement('section');featured.id='featured';featured.className='section';main.prepend(featured);}
  ...
  tabs.addEventListener('click',e=>{
    ...
    setTimeout(()=>setInterstitialVisibility(tab.dataset.view==='today'),0);
  },true);

  const observer=new MutationObserver(()=>purgeLeak());
  observer.observe(document.body,{childList:true,subtree:true});
  setTimeout(purgeLeak,250);
  setTimeout(purgeLeak,1000);
}
if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>setTimeout(enforce,100),{once:true});
else setTimeout(enforce,100);
})();