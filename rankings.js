...
function baseOpen(x){
 if(x?.mission_score_status==='rated'&&x?.mission_score!=null)return {score:clamp(x.mission_score),kind:'native',evidence:'Mission Rated reviews'};
 const signals=Math.min(100,(Number(x?.signal_count)||0)*12),school=x?.school_liaison?100:0,pcs=x?.pcs_support?100:0,official=x?.official_verified?100:0,parts=[[signals,.40],[school,.25],[pcs,.25],[official,.10]].filter(([v])=>v>0);
...
function applyAll(j){
 ...
 if(path.endsWith('/bases.html')){addLegend(document.querySelector('main .wrap'),'derived from verified PCS/family-readiness evidence currently available. This is not a base quality-of-life review score.');applyCards(document.getElementById('grid'),j.installations,baseOpen,'bases');return}
 ...
 addLegend('bases','derived from verified PCS/family-readiness evidence currently available; it is not a base quality-of-life review score.');
...
async function run(){style();let j;try{const r=await fetch(API,{headers:{accept:'application/json'}});if(!r.ok)return;j=await r.json()}catch{return}let tries=0;const timer=setInterval(()=>{applyAll(j);if(++tries>=8)clearInterval(timer)},400);applyAll(j)}
