...
// Buy a Car is a first-class Mission Rated Live view. Keep the legacy route only as the embedded implementation/detail target.
if(p==='/'){
 ...
 if(row&&main&&!document.querySelector('[data-view="cars"]')){
  ...
  const section=document.createElement('section');section.className='section';section.id='cars';section.hidden=true;section.innerHTML='<div class="head"><div><h2>Buy a Car</h2><small>DEALERS • SALESPEOPLE • DEAL CHECK</small></div></div><div class="mr-car-note">Dealer ratings and sourced salesperson mentions stay separate from Mission Rated reputation. Thin evidence remains MR Building.</div><iframe class="mr-car-frame" title="Mission Rated Buy a Car" loading="eager" src="/buy-a-car.html?embedded=1"></iframe>';main.appendChild(section);
  const activate=()=>{
