...
nav.innerHTML=items.map(x=>{const active=x.match.some(m=>p===m||p===m+'.html')||(p==='/'&&((x.label==='CARS'&&liveView==='cars')||(x.label==='SUPPORT'&&liveView==='support')));return `<a href="${x.href}"${active?' aria-current="page"':''}><b aria-hidden="true">${x.icon}</b>${x.label}</a>`}).join('');
document.body.appendChild(nav);

if(p==='/'){
 const row=document.querySelector('.tabrow'),main=document.querySelector('main.main');
 if(row&&main){
  const installView=({id,label,small,note,src,messageType})=>{
   ...
   const frame=section.querySelector('.mr-embed-frame');
   window.addEventListener('message',e=>{if(e.origin!==location.origin||e.source!==frame.contentWindow||e.data?.type!==messageType)return;const h=Number(e.data.height);if(Number.isFinite(h)&&h>300&&h<7000)frame.style.height=`${h}px`});
  ...
  const support=installView({id:'support',label:'Support',small:'FAMILY • MONEY • CAREER • MOVING • DEPLOYMENT',note:'Find authoritative military and family support resources without leaving Mission Rated Live. Every resource remains MR Building until Mission Rated has enough first-party evidence for its own score.',src:'/support.html?embedded=1',messageType:'mr-support-height'});
  const cars=installView({id:'cars',label:'Buy a Car',small:'DEALERS • SALESPEOPLE • DEAL CHECK',note:'Start with people worth buying from. Dealer ratings and sourced salesperson mentions stay separate from Mission Rated reputation; thin evidence remains MR Building.',src:'/buy-a-car.html?embedded=1',messageType:'mr-car-height'});
  if(liveView==='support')support?.activate();else if(liveView==='cars')cars?.activate();
 }
}
})();
