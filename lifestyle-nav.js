...
const dirDest=x=>[x?.address_line1||x?.address,x?.city,x?.state,x?.postal_code].map(v=>String(v??'').trim()).filter(Boolean).join(', ')||[x?.name,x?.city,x?.state].map(v=>String(v??'').trim()).filter(Boolean).join(', ');
...
(async()=>{try{const ROOT='https://vquwdypidgjmxnhhdbol.supabase.co/functions/v1/';const [e,m]=await Promise.allSettled([fetch(ROOT+'public-explore').then(r=>r.ok?r.json():null),fetch(ROOT+'public-medical').then(r=>r.ok?r.json():null)]);const explore=e.status==='fulfilled'&&e.value?e.value:{},medical=m.status==='fulfilled'&&m.value?m.value:{};const bm=new Map((explore.businesses||[]).map(x=>[dirKey(x.name),x])),im=new Map((explore.installations||[]).map(x=>[dirKey(x.name),x])),mm=new Map((medical.providers||[]).map(x=>[dirKey(x.name),x]));const apply=()=>{if(p==='/'){applyDirections(document.getElementById('businessGrid'),bm);applyDirections(document.getElementById('baseGrid'),im)}if(p==='/medical'||p==='/medical.html')applyDirections(document.getElementById('grid'),mm);if(p==='/bases'||p==='/bases.html')applyDirections(document.getElementById('grid'),im);if(p==='/business'||p==='/business.html'){const item=bm.get(dirKey(document.getElementById('name')?.textContent));if(item)addDirections(document.querySelector('#content .card'),item)}if(p==='/installation'||p==='/installation.html'){const item=im.get(dirKey(document.querySelector('h1')?.textContent));if(item)addDirections(document.querySelector('.card'),item)}};apply();let n=0;const t=setInterval(()=>{apply();if(++n>18)clearInterval(t)},350)}catch{}})();
})();