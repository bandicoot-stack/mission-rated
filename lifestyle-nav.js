...
const addDirections=(card,item)=>{if(!card||!item||card.querySelector('.mrDirections'))return;const href=dirUrl(item);if(!href)return;const a=document.createElement('a');a.className='mrDirections';a.href=href;a.target='_blank';a.rel='noopener noreferrer';a.textContent='↗ Directions';a.setAttribute('aria-label',`Directions to ${item.name||'this location'}`);const links=card.querySelector('.links,.actions');if(links)links.appendChild(a);else card.appendChild(a)};
...
(async()=>{try{const ROOT='https://vquwdypidgjmxnhhdbol.supabase.co/functions/v1/';const [e,m,d]=await Promise.allSettled([fetch(ROOT+'public-explore').then(r=>r.ok?r.json():null),fetch(ROOT+'public-medical').then(r=>r.ok?r.json():null),fetch(ROOT+'public-directions').then(r=>r.ok?r.json():null)]);const explore=e.status==='fulfilled'&&e.value?e.value:{},medical=m.status==='fulfilled'&&m.value?m.value:{},directions=d.status==='fulfilled'&&d.value?d.value:{};const locationBusinesses=new Map((directions.businesses||[]).map(x=>[dirKey(x.name),x])),locationInstallations=new Map((directions.installations||[]).map(x=>[dirKey(x.name),x]));const bm=new Map((explore.businesses||[]).map(x=>[dirKey(x.name),{...x,...(locationBusinesses.get(dirKey(x.name))||{})}])),im=new Map((explore.installations||[]).map(x=>[dirKey(x.name),{...x,...(locationInstallations.get(dirKey(x.name))||{})}])),mm=new Map((medical.providers||[]).map(x=>[dirKey(x.name),{...x,...(locationBusinesses.get(dirKey(x.name))||{})}]));const apply=()=>{if(p==='/'){applyDirections(document.getElementById('businessGrid'),bm);applyDirections(document.getElementById('baseGrid'),im)}if(p==='/medical'||p==='/medical.html')applyDirections(document.getElementById('grid'),mm);if(p==='/bases'||p==='/bases.html')applyDirections(document.getElementById('grid'),im);if(p==='/business'||p==='/business.html'){const item=bm.get(dirKey(document.getElementById('name')?.textContent));if(item)addDirections(document.querySelector('#content .card'),item)}if(p==='/installation'||p==='/installation.html'){const item=im.get(dirKey(document.querySelector('h1')?.textContent));if(item)addDirections(document.querySelector('.card'),item)}};apply();let n=0;const t=setInterval(()=>{apply();if(++n>18)clearInterval(t)},350)}catch{}})();
})();